"""Round 7: probe whether hidden=512 also lifts d=3 (round 6 showed it
dramatically helps d=2 / ε=0.20). Decides whether the production CPU
rerun on della should use hidden=512 across all dims or only at d=2.

Same FB config as round 6 minus the dim:
    z=8, hidden=512, n_traj=500, n_epochs=300, gamma=0.9, batch=1024,
    cosine_lr, lr=1e-4
τ ∈ {0.05, 1.0} (matching round 6 — single FB train, two readouts).
d=3, ε ∈ {0.01, 0.05, 0.10, 0.20}, 15 manifolds.

15 × 4 = 60 FB trains × ~60 s ≈ 60 min on local cuda:0.
"""
...
CSV = OUT_DIR / "round7_d3_hidden512.csv"
    ...
    for noise in NOISES:
        with warnings.catch_warnings():
            ...
            cc = TauColosseum(
                ...
                seed=2024,
                save_directory=f"/tmp/.tau-r7-d{DIM}-n{noise:g}",
                **DATASET_CFG,
            )
