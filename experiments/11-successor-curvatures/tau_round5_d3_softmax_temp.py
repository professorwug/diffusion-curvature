"""Round 5: same softmax-τ sweep as round 4, but at d=3.

Round-3 d=3 mean ρ was 0.40 with default τ=1.0. Round 4 (d=2) showed that
lower τ amplifies dynamic range; this round tests whether the same trick
helps at d=3.

Same FB config: z=8, hidden=256, gamma=0.9, n_traj=500, n_epochs=300,
batch=1024, cosine_lr, lr=1e-4. τ ∈ {0.05, 0.1, 0.3, 1.0, 3.0}, 4 noise
levels, 20 manifolds.
"""
...
CSV = OUT_DIR / "round5_d3_softmax_tau.csv"
...
def main() -> None:
    ...
    for noise in NOISES:
        with warnings.catch_warnings():
            ...
            cc = TauColosseum(
                ...
                seed=2024,
                save_directory=f"/tmp/.tau-r5-d{DIM}-n{noise:g}",
                **DATASET_CFG,
            )
    ...
    df = pd.read_csv(CSV)
    print(f"\n========== Pearson r per (softmax τ × ε) at d={DIM} ==========")
