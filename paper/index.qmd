...
Ollivier Ricci Curvature is based on an invariant property of flat spaces, the deviation from which allows for a measure of curvature. However, the elegance of this invariance-based definition comes at a computational cost. Ollivier Ricci, and related methods like Forman Ricci, require optimal transport calculations for (a subset) of each pair of edges, a complexity growing with $O(n^2)$. Additionally, the transport distance based on the graph shortest-path distance is highly susceptible to noise.
...
Our previous paper [@bhaskar2022DiffusionCurvatureEstimating] measured this laziness directly, as the return probability within a k-neighborhood of the starting point. This required tuning the parameter $k$, and neglected the information provided by the random walk probabilities outside of this k-neighborhood. We now improve this definition in two ways.
...
Ollivier Ricci curvature measures the extent to which 'spheres are closer than their centers', using graph diffusions to create spheres, and optimal transport to measure distances. Diffusion Laziness uses both ingredients. But whereas Ollivier Ricci curvature takes the Wasserstein distance between spheres of the same size, Diffusion Laziness takes the distance between successive sizes of the same sphere.

This simplification makes Diffusion Laziness more localized and much computationally lighter. Here we demonstrate that it also retains the theoretical guarantees of Ollivier Ricci curvature, and might be though of as a natural node-wise adaptation of Ollivier Ricci curvature.

::: {#prp-coarse-ricci-curvature-bounds-diffusion-laziness}
#### Coarse Ricci Curvature Bounds Diffusion Laziness

Let $(X,d,m)$ be a metric space equipped with a random walk, with coarse Ricci curvature bounded from below by some $k$ such that $\kappa(x,y) \geq k$ for all $x,y \in X$. The diffusion laziness of a $t$ step diffusion in $X$ is bounded above by
$$
...
For a single graph, the laziness of diffusion is linked to graph volume and theoretically bounded by curvature. Within a given graph, the higher the curvature, the higher the diffusion laziness. As shown above, we can draw analogy to Bishop-Gromov to convert this unsigned magnitude-of-curvature estimation to a signed curvature via a comparison between spaces. However, this assumes that the diffusions across spaces have the same units – that the time $t$ operates on the same scale.
