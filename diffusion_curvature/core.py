...
# %% auto 0
__all__ = ['graphtools_graph_from_data', 'SimpleGraph', 'get_adaptive_graph', 'get_fixed_graph', 'DiffusionCurvature']
...
# %% ../nbs/0c-Core.ipynb 9
from .diffusion_laziness import DiffusionLaziness, compare_curvature_across_datasets_by_locality_fraction
...
class DiffusionCurvature():
    ...
    def __init__(
            self,            
            graph_former = None, # a function which turns a pointcloud into a graph with an adjacency matrix
            smoothing = 2, # Amount of local averaging to perform on calculated curvature. Recommend: 1 or 2.
            estimate_local_dimension:bool = True, # by default, estimates the dimension of each point. If false, estimates global dimension
            diffusion_type:DIFFUSION_TYPES = 'diffusion matrix', # Either ['diffusion matrix','heat kernel']
            laziness_method: LAZINESS_METHOD = 'Entropic', # Either ['Wasserstein','Entropic', 'Laziness']
            comparison_method: COMPARISON_METHOD = 'Subtraction', # Either ['Ollivier', 'Subtraction']
            comparison_space_trials = 3, # Number of trials
    ):
        store_attr()
        self.D = None
    ...
    def fit(
            self,
            X, # input pointcloud
            locality_scale = 0.9, # the scale at which to measure curvature, from 0 to 1. 0 is the most local possible, 1 is the most global possible. Generally should be close to 1.
            dim = None, # the INTRINSIC dimension of your manifold, as an int for global dimension or list of pointwise dimensions; if none, tries to estimate pointwise.
            idx=None, # the index at which to compute curvature. If None, computes for all points.
            ts:int = list(range(1,50)), # number of scales of diffusion; supply an integer or list of indices, and we'll only calculate their laziness. This is a good default.
            unsigned = False, # If True, computes unsigned curvature. If False, computes signed curvature.
            D = None, # Supply manifold distances yourself to override their computation. Only used with the Wasserstein laziness method.
            t = None, # alias for ts
            graph_knn = 150, # number of nearest neighbors to use in graph construction, with tuned curvature agnostic kernel
    ):
        # build graph
        ...
        if self.graph_former is None:
            self.graph_former, _ = tune_curvature_agnostic_kernel(X, graph_knn, tolerance = 5)
        ...
        if dim is None:
            if self.estimate_local_dimension:
                ...
                dim = int(danco.dimension_)
                dim = [dim]*len(X)
            print(f"Estimated dimension(s) in point cloud: {set(dim)}")
        if isinstance(dim, int):
            dim = [dim]*len(X)
        else:
